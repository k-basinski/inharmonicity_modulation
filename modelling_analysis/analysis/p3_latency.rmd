...
writeLines("REPEATED MEASURES ANOVA TO CHECK WHETER P3 LATENCIES DIFFER SIGNIFICANTLY BETWEEN HARMONICITY CONDIONS", file_connection)
...
# P3 latency - pairwise contrasts with condition jitter 0

Here we check whether P3 latency in condition with no jitter differs from any jittered condtions.
...
for (j in jitters_names[2:8]) {
  data_x <- dane[dane$jitter == j, "p3_lat"]
  data_y <- dane[dane$jitter == "j0", "p3_lat"]
...
# P3 latency - null model


```{r}
p3_lat_null_m <- lme(p3_lat ~ 1,
                       data = dane_excl0,
                       random = ~1 | pid,
                      method = "ML"
)

summary(p3_lat_null_m)
```

```{r include = FALSE}
save_mixed_model(summary(p3_lat_null_m), "00_null_model", output_dir, nice_tables_file)
```


# P3 latency - linear model

```{r}
p3_lat_line_m <- lme(p3_lat ~ poly(jitter_support_log10, 1),
                      dane_excl0,
                      random = ~1 | pid,
                      method = "ML")

summary(p3_lat_line_m)
```

```{r include = FALSE}
save_mixed_model(summary(p3_lat_line_m), "01_linear_model", output_dir, nice_tables_file)
```

# P3 latency - quadratic model

```{r}
p3_lat_quad_m <- lme(p3_lat ~ poly(jitter_support_log10, 2),
                      dane_excl0,
                      random = ~1 | pid,
                      method = "ML")

summary(p3_lat_quad_m)
```

```{r include = FALSE}
save_mixed_model(summary(p3_lat_quad_m), "02_quadratic_model", output_dir, nice_tables_file)
```


# P3 latency - qubic model

```{r}
p3_lat_qube_m <- lme(p3_lat ~ poly(jitter_support_log10, 3),
                      dane_excl0,
                      random = ~1 | pid,
                      method = "ML")

summary(p3_lat_qube_m)
```

```{r include = FALSE}
save_mixed_model(summary(p3_lat_qube_m), "03_qube_model", output_dir, nice_tables_file)
```

# P3 latency - pow4 model

```{r}
p3_lat_pow4_m <- lme(p3_lat ~ poly(jitter_support_log10, 4),
                      dane_excl0,
                      random = ~1 | pid,
                      method = "ML")

summary(p3_lat_pow4_m)
```

```{r include = FALSE}
save_mixed_model(summary(p3_lat_pow4_m), "04_pow4_model", output_dir, nice_tables_file)
```

# MMN_amp model comparison - polynomial

```{r}
aov_table_poly <- anova(p3_lat_null_m, p3_lat_line_m, p3_lat_quad_m, p3_lat_qube_m, p3_lat_pow4_m)
```
