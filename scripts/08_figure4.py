...
# get unified colormap from mpl, not seaborn
viri = mpl.colormaps["viridis"](np.linspace(0, 1, 8))
# %%
# load data
p3_predictions = pd.read_csv(
    "../modelling_analysis/output/p3_amp/model_predictions.csv"
)
...
# model parameters
linear_model = {"intercept": 0.4155487, "slope": 0.9358146}
sigmoid_model = {"L": -1.310175, "I_0": -1.176047, "k": 6.244479}

data_means = (
    peak_measures_df.groupby("jitter")
    .agg(
        {
            "jitter_no": "mean",
            "jitter_value": "mean",
...
standard_error_mmn = data_means[("mmn_amp", "std")] / np.sqrt(
    data_means[("mmn_amp", "size")]
)
standard_error_p3 = data_means[("p3_amp", "std")] / np.sqrt(
    data_means[("p3_amp", "size")]
)
data_means[("mmn_amp", "CI95_LL")] = (
    data_means[("mmn_amp", "mean")] - t_95quant * standard_error_mmn
)
data_means[("mmn_amp", "CI95_UL")] = (
    data_means[("mmn_amp", "mean")] + t_95quant * standard_error_mmn
)
data_means[("p3_amp", "CI95_LL")] = (
    data_means[("p3_amp", "mean")] - t_95quant * standard_error_p3
)
data_means[("p3_amp", "CI95_UL")] = (
    data_means[("p3_amp", "mean")] + t_95quant * standard_error_p3
)
...
curve_y_mmn = sigmoid_model["L"] / (
    1 + (np.exp(sigmoid_model["k"] * (x - sigmoid_model["I_0"])))
)
...
# boxplots for means
sns.boxplot(
    peak_measures_df, x="jitter", y="mmn_amp", ax=ax[0][0], palette=viri, saturation=1
)
sns.boxplot(
    peak_measures_df, x="jitter", y="p3_amp", ax=ax[0][1], palette=viri, saturation=1
)
...
for pid in peak_measures_df["pid"].unique():
    ...
    ax[1][0].plot(
        this_data["jitter_support_log10"],
        this_data["mmn_amp"],
        "-",
        alpha=0.5,
        linewidth=1,
...
for v, j in zip(peak_measures_df["jitter_support_log10"].unique()[1:], jits):
    ...
    ax[1][0].annotate(j, (v + 0.01, -4.5))
    ax[1][1].annotate(j, (v + 0.01, -3))
...
ax[0][0].set_ylabel("MMN mean amplitude (µV)")
ax[0][1].set_ylabel("P3a mean amplitude (µV)")

ax[1][0].set_xlabel("differential entropy")
ax[1][1].set_xlabel("differential entropy")
ax[1][0].set_ylabel("MMN mean amplitude (µV)")
ax[1][1].set_ylabel("P3a mean amplitude (µV)")
